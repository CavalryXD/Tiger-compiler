...
public class TranslateVisitor implements ast.Visitor {
  ...
  private ClassTable table;     // <class_name, class_binding> 辅助了classes和vtables的生成
  ...
  private LinkedList<Dec.T> tmpVars;        // 指向class的struct指针集合
  private LinkedList<Class.T> classes;      // 和vtables会在优美打印中使用
  ...
  public void visit(ast.Ast.MainClass.MainClassSingle c) {
      ...
      c.stm.accept(this);       // 在这里面可能会往tmpVars里面加Dec对象, 这样locals中就会有struct的指针
      // todo
